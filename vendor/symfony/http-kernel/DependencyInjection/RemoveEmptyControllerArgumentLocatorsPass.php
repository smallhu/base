...
/**
 * Removes empty Services-locators registered for ServiceValueResolver.
 *
...
class RemoveEmptyControllerArgumentLocatorsPass implements CompilerPassInterface
{
    ...
    public function __construct($resolverServiceId = 'argument_resolver.Services')
    {
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($controllers as $controller => $argumentRef) {
            ...
            if (!$argumentLocator->getArgument(0)) {
                ...
                $reason = sprintf('Removing Services-argument resolver for controller "%s": no corresponding services exist for the referenced types.', $controller);
            } else {
                ...
                foreach ($controllerDef->getMethodCalls() as list($method, $args)) {
                    if (0 === strcasecmp($action, $method)) {
                        $reason = sprintf('Removing method "%s" of Services "%s" from controller candidates: the method is called at instantiation, thus cannot be an action.', $action, $id);
                        break;
                    }
                }
                if (!$reason) {
